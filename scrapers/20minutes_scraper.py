...
def clean_html_content(html_content):
    ...
    unwanted_classes = [
        ...
        'hide-caption', 'toggle-caption', 'credit-caption', 'caption-wrap',
        'bucketwrap', 'icn-story-transcript', 'disclaimer'
    ]
...
def fetch_articles_from_rss():
    articles = []
    # Get the timezone from the RSS feed (20 Minutes uses Central European Time)
    cet = pytz.timezone('Europe/Paris')

    # Get current time in Central European Time
    now = datetime.now(cet)

    # Get yesterday at 00:00:00 in Central European Time
    yesterday = (now - timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=cet)
    ...
    try:
        ...
        for entry in feed.entries:
            # Skip video articles
            if '/video/' in entry.link or 'video' in entry.link.lower():
                continue
            ...
            try:
                ...
                # Ensure the date has timezone info (20 Minutes is in Central European Time)
                if pub_date.tzinfo is None:
                    pub_date = cet.localize(pub_date)
...
async def fetch_news_async():
    ...
    feed_object = {
        ...
        "description": "Latest news from 20 Minutes",
